...
final class CondaPackagesTable {
    ...
    private void refreshPackages(final boolean isInitialRefresh) {
        ...
        List<CondaPackageSpec> removed;
        List<CondaPackageSpec> unconfigured;
        try {
            ...
            if (isInitialRefresh) {
                ...
                final Set<CondaPackageSpec> excludedSet = new HashSet<>(m_config.getExcludedPackages());
                included = new ArrayList<>();
                excluded = new ArrayList<>();
                removed = new ArrayList<>();
                unconfigured = new ArrayList<>();
                ...
                Set<CondaPackageSpec> packagesSet = new HashSet<>(packages);
        ...
        final List<CondaPackageSpec> removedFinal = removed;
        final List<CondaPackageSpec> unconfiguredFinal = unconfigured;
        if (!removed.isEmpty()) {
            invokeOnEDT(() -> {
                final int numPackages = removedFinal.size();
        ...
        if (!unconfigured.isEmpty()) {
            invokeOnEDT(() -> {
                final int numPackages = unconfiguredFinal.size();
                final boolean pl = numPackages > 1;
        ...
        invokeOnEDT(() -> m_model.setPackages(includedFinal, excludedFinal, removedFinal, unconfiguredFinal));
